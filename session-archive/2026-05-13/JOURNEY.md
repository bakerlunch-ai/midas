...
Phase 1 (infra foundation) ──────────► ✅ DONE (May 12, end of session 6)
   ↓
Phase 1.5 (architectural cleanup)     ✅ 3/3 closed; 9 minor carry items remain
   ↓
Phase 2 (data layer: data-svc, oms-svc, pms-svc) ──► you are here
   - Event vocabulary: ✅ 7/7 types defined
   - data-svc: 🟡 6/8 build-pieces done (session 7)
   - oms-svc + pms-svc: 0/2 not started
   ↓
...
**Phase 2 services status (after session 7):**

**data-svc** — 🟡 6 of ~8 build-pieces done. Phase 2's first service is two manifests + a sealed-secret away from being live in the cluster.

| Piece | What | Status |
|---|---|---|
| 1 | Workspace scaffold + read-only KalshiClient + 11 tests | ✅ commit `51e52a2` (session 6) |
| 2 | FastAPI `main.py` with lifespan-managed KalshiClient | ✅ commit `07aa840` |
| 3 | NATSPublisher + lifespan wiring | ✅ commit `eb43e3c` |
| 4 | Tick poller — first service that *actively emits events* | ✅ commit `b821e83` |
| 5 | Dockerfile + GitHub Actions build pipeline | ✅ commit `7b6cbc9` |
| 6 | K8s manifests (Deployment, Service, Namespace) | ✅ commit `a6ec221` |
| 7 | Sealed-secret for Kalshi credentials | ⏳ next session |
| 8 | Argo Application + deploy + verify ticks on NATS | ⏳ next session |

**oms-svc** — not started. Will follow data-svc's canonical shape (lifespan-managed connections, fail-loud probes, defensive parsing, hierarchical NATS subjects).

**pms-svc** — not started. Same shape.

**Session 7 milestone — first service that does work on its own:** The tick poller commit (`b821e83`) is the single most-consequential commit of the Midas rebuild so far. Before tonight every commit had been scaffolding; this is the first one where one of the new services actively produces events. Once piece 7 + piece 8 land next session, ticks will flow on NATS subject `events.market.tick.kalshi.<ticker>` for real — the proof-of-life moment for the v2 architecture.

**v2 architecture reference:** §02 (service map), §03 (event sourcing), §04 (bot-events package — completed in session 6), §05 (data-svc), §06 (database schemas), §07 (Kubernetes deployment), §10 (CI/CD).
...
| Phase 1.5 progress | **✅ 3 / 3 architectural problems closed; 9 minor carry items remain** |
| Phase 2 progress | Event vocabulary: **7/7 types defined**. data-svc: **6/8 build-pieces done**. oms-svc + pms-svc: 0/2 not started. |
| Calendar elapsed | ~14 days (Apr 29 → May 13) |
| Time invested | ~32 hours across 7 sessions |
| Cloud cost | ~$130 / month (target: <$150) |
| Commits on main | 61 (53 going into session 7 → 8 added tonight) |
| Latest commit | `a6ec221` — deploy(data-svc): add K8s manifests (Deployment, Service, Namespace) |
| Tests | 156 passing (141 → 156 in session 7) |
| Services running | hello-svc + full Phase 1 infra. data-svc image built and in GHCR; manifests in git; pod not yet running. |
| Argo Applications | 9 total (7 Synced+Healthy, 2 OutOfSync but cosmetic; data-svc Application not yet committed) |

**End-of-session 7 milestone — first service that actively generates events:** session 7's tick-poller commit (`b821e83`) is the first commit on Midas where one of the new services *does work on its own*, not just *is structurally able to*. Combined with the Docker image (`7b6cbc9`) and K8s manifests (`a6ec221`) that landed the same night, data-svc is now two commits away from being a live, deployed, event-emitting service in the cluster. Next session ships those two commits (sealed-secret + Argo Application). After that, Midas has its first deployed Phase 2 service producing observable data — the proof-of-life moment for the v2 architecture.

**Update cadence:** this file is updated when phase progress materially changes (a phase completes, a task count shifts) or when a phase's "what it enables" section changes because of new findings. Not every session touches it. Session 7 touched it because Phase 2 entered active service-build territory, the first service crossed the threshold from "capable" to "actively producing events," and the data-svc piece tracker is the right place to anchor the next session's planning.
