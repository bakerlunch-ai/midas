...
|---|---|---|---|---|---|
| **2026-05-28** | ~1.5h | **CI restored + logging fix.** Set out to add a one-line logging fix; discovered CI had been silently RED on every commit since ~2026-05-21 — `make lint` was failing on 11 accumulated ruff errors, which blocked `make test` from ever running. The Build workflow (which Argo follows) is independent of CI, so deploys kept shipping on green "Build" while "CI" was cosmetic. Fixed in three honest commits: auto-fix 10 ruff errors (`5479316`, CI still red), hand-fix the last SIM117 error (`cf893a8`, **CI green again — test step ran for first time in a week**), then the original logging fix (`d605909`). data-svc redeployed; `tick_poller` poll-cycle logs now stream in `kubectl logs` (`published=0 skipped=20` visible live). Locked the market-selection design decision for next session. | +3 (5479316 → d605909) | Phase 2: data-svc operational + observable; CI gate restored. | [SESSION_REPORT_2026-05-28.html](SESSION_REPORT_2026-05-28.html) |
...
|---|---|---|---|---|---|---|---|
| 2026-05-28 (after session 9) | ✅ 12 / 12 | ✅ 3 / 3 + 9 carry | Vocab 7/7, **data-svc 8/8 LIVE + observable**, oms/pms 0/2 | ~$130 | 71 | 161 | hello-svc + **data-svc** (pod Running, poll-cycle logs visible, ticks verified) + Phase 1 infra. **CI gate restored (green).** |
...
- **Session 9 note:** a "10-minute logging fix" surfaced a structural hole — CI had been silently red for ~6 commits since 2026-05-21, the literal subject of Lesson 10 ("the bot grew faster than its tests"). `make lint` failing blocked `make test` from running, and because the Build workflow is independent of CI, deploys never noticed. Fixed the gate before shipping the feature: cleared lint debt in two honest commits, confirmed CI green (test step ran for the first time in a week), then landed the logging fix on the verified baseline. The discipline echo: a gate nobody watches is the same as no gate — exactly Lesson 2's reconciliation failure, in a new costume. Market-selection design decided (ingest-all → rule-filter → liquidity-weighted) for session 10 implementation.
